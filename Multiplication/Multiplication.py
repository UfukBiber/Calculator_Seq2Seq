...
OutLength = len(str(MaxNumber*MaxNumber)) 
    ...
    while len(Inp) < quantity:
        ...
        if notInclude([x, y], allData):
            ...
            if len(outputMultiplication) < OutLength:
                outputMultiplication = "_"*(OutLength-len(outputMultiplication)) + outputMultiplication
            x = str(x)
            y = str(y)
            if len(x) < InpLength:
...
if os.path.exists("my_model_0"):
    model = tf.keras.models.load_model("my_model_0")
